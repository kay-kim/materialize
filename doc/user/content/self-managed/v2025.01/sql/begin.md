...
Value | Description
------|----------
`ISOLATION LEVEL <level>` | Sets the transaction [isolation level](/self-managed/v2025.01/overview/isolation-level).
`READ ONLY` | Limits the transaction to read-only operations.

## Details

`BEGIN` starts a transaction block.
All statements in a transaction block will be executed in a single transaction until an explicit [`COMMIT`](/self-managed/v2025.01/sql/commit) or [`ROLLBACK`](/self-managed/v2025.01/sql/rollback) is given.
...
A **read-only** transaction starts with a [`SELECT`](/self-managed/v2025.01/sql/select) statement and allows only `SELECT` statements.
...
A second kind of **read-only** transaction can contain an initial [`SUBSCRIBE`](/self-managed/v2025.01/sql/subscribe), which can appear in a transaction block along with [`DECLARE`](/self-managed/v2025.01/sql/declare) and [`FETCH`](/self-managed/v2025.01/sql/fetch).

### Write-only transactions

A **write-only** transaction starts with an [`INSERT`](/self-managed/v2025.01/sql/insert) and allows only `INSERT` statements.
