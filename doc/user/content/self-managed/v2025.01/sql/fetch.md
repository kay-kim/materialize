...
`FETCH` retrieves rows from a query using a cursor previously opened with [`DECLARE`](/self-managed/v2025.01/sql/declare).
...
------------|------------|---------|----------
`timeout`   | `interval` | None    | When fetching from a [`SUBSCRIBE`](/self-managed/v2025.01/sql/subscribe) cursor, complete if there are no more rows ready after this timeout. The default will cause `FETCH` to wait for at least one row to be available.
...
For [`SUBSCRIBE`](/self-managed/v2025.01/sql/subscribe) queries, `FETCH` by default will wait for rows to be available before returning.
