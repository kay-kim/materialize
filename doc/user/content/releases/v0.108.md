...
* Allow specifying the message key format and the message value format
  separately in [Kafka sinks](/self-managed/v2025.01/sql/create-sink/kafka/), using the new `KEY
  FORMAT ... VALUE FORMAT ...` option.

* Support including a header row in `CSV` files exported using [S3 bulk exports](/self-managed/v2025.01/sql/copy-to/#copy-to-s3).
...
* Add `hydration_time` to the [`mz_internal.mz_compute_hydration_statuses`](/self-managed/v2025.01/sql/system-catalog/mz_internal/#mz_compute_hydration_statuses)
