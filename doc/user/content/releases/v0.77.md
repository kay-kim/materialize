...
* Support the `now()` function in the `CHECK` expression of [webhook sources](/self-managed/v2025.01/sql/create-source/webhook/).
...
* Add [`mz_internal.mz_materialization_lag`](/self-managed/v2025.01/sql/system-catalog/mz_internal/#mz_materialization_lag)
...
* Improve the output format of [`EXPLAIN...PLAN AS TEXT`](/self-managed/v2025.01/sql/explain-plan/) when the `humanized_exprs`
  [output modifier](/self-managed/v2025.01/sql/explain-plan/#output-modifiers) to avoid ambiguities when
