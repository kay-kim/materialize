...
* Add support for new SQL functions:
  ...
  | ------------------------------------------------------------------ | ----------------------------------------------------------- |
  | [`mz_is_superuser`](/self-managed/v2025.01/sql/functions/#access-privilege-inquiry-functions) |  Reports whether the current role is a _superuser_ with administration privileges in Materialize. |
  | [`regexp_replace`](/self-managed/v2025.01/sql/functions/#string-functions) | Replaces the first occurrence of the specified regular expression in a string with the specified replacement string.    |
  | [`regexp_split_to_array`](/self-managed/v2025.01/sql/functions/#string-functions)      | Splits a string by the specified regular expression into an array. |
  | [`regexp_split_to_table`](/self-managed/v2025.01/sql/functions/#table-functions)       | Splits a string by the specified regular expression.               |
...
* Make the syntax for [`GROUP SIZE` query hints](/self-managed/v2025.01/transform-data/optimization/#query-hints)
...
* Add `savings` to the [`mz_internal.mz_expected_group_size_advice`](/self-managed/v2025.01/sql/system-catalog/mz_introspection/#mz_expected_group_size_advice)
  system catalog table. This column provides a conservative estimate of the memory
  savings that can be expected by using [`GROUP SIZE` query hints](/self-managed/v2025.01/transform-data/optimization/#query-hints).
