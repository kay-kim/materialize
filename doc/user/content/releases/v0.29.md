...
* Treat some errors as transient instead of fatal in the [PostgreSQL source](/self-managed/v2025.01/sql/create-source/postgres/).
...
* Allow users to create indexes on system objects to optimize the performance of
  [troubleshooting](/self-managed/v2025.01/ops/troubleshooting/) queries.

* Include indexes created on system objects when running the [`SHOW INDEXES`](/self-managed/v2025.01/sql/show-indexes)
  command if the `IN CLUSTER` clause is specified.

* Add a `TPCH` [load generator source](/self-managed/v2025.01/sql/create-source/load-generator/#tpch),
