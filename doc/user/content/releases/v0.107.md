...
* Support exporting data to Google Cloud Storage (GCS) using [AWS connections](/self-managed/v2025.01/sql/create-connection/#aws)
  and the [`COPY TO`](/self-managed/v2025.01/sql/copy-to/) command. While Materialize does not natively
  ...
  which allows GCP users to take advantage of [S3 bulk exports](/self-managed/v2025.01/sql/copy-to/#copy-to-s3)
  also for GCS.

#### SQL

* Add the [`@>` and `<@` operators](/self-managed/v2025.01/sql/types/list/#list-containment), which
...
* Add `connection_id` to the [mz_internal.mz_sessions](/self-managed/v2025.01/sql/system-catalog/mz_internal/#mz_sessions)
...
* Move the `PROGRESS TOPIC REPLICATION FACTOR` option to the `CREATE CONNECTION`
  command for [Kafka connections](/self-managed/v2025.01/sql/create-connection/#kafka)
