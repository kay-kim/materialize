...
* **Breaking change.** Disallow specifying more starting offsets than the number
    of partitions for [Kafka sources](/self-managed/v2025.01/sql/create-source/kafka/#setting-start-offsets).

* Allow configuring the group ID (`GROUP ID PREFIX`) for [Kafka sources](/self-managed/v2025.01/sql/create-source/kafka/#connection-options),
  and the group ID and transactional ID (`TRANSACTIONAL ID PREFIX`, `PROGRESS GROUP ID PREFIX`)
  for [Kafka sinks](/self-managed/v2025.01/sql/create-sink/kafka/#connection-options)).
...
* Add [mz_internal.mz_notices](/self-managed/v2025.01/sql/system-catalog/mz_internal/#mz_notices) to
