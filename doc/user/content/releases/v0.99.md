...
* **Private preview.** Support exporting objects and query results to Amazon s3
    using the [`COPY TO`](/self-managed/v2025.01/sql/copy-to/) command and [`AWS connections`](/self-managed/v2025.01/sql/create-connection/#aws).
...
* Support ingesting datetime columns as text via the `TEXT COLUMNS` option in
  the [MySQL source](/self-managed/v2025.01/sql/create-source/mysql/) to work around MySQL's _zero_
...
* **Private preview.** Support setting a history retention period for sources,
    tables, materialized views, and indexes via the new [`RETAIN HISTORY`](/self-managed/v2025.01/transform-data/patterns/durable-subscriptions/#history-retention-period)
    option. This is useful to implement [durable subscriptions](/self-managed/v2025.01/transform-data/patterns/durable-subscriptions/).
