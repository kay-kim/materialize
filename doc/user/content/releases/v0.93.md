...
* Do not error if the `oid` or `typmod` of a column changes when using the `TEXT COLUMNS`
  option to ingest data as text in a [PostgreSQL source](/self-managed/v2025.01/sql/create-source/postgres/).
...
* Extend `pg_catalog` coverage with support for the [`obj_description()`](/self-managed/v2025.01/sql/functions/#obj_description)
