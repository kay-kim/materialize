...
* Support using the new `NULL DEFAULTS` option in Avro-formatted [Kafka sinks](/self-managed/v2025.01/sql/create-sink/).
When specified, this option will generate an Avro schema where every nullable
field has a default of `NULL`.

#### SQL

* Add the [`EXPLAIN CREATE { MATERIALIZED VIEW | INDEX }`](/self-managed/v2025.01/sql/explain-plan/#explained-object)
