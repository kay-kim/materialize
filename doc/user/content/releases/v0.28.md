...
* Use [static IPs](/self-managed/v2025.01/ops/network-security/static-ips/) when initiating connections from sources
  and sinks. You can use these IPs in your firewall configuration to authorize
  connections from your Materialize region.

* Improve the syntax for [`CREATE CONNECTION`](/self-managed/v2025.01/sql/create-connection). The
...
* Improve the usability of the `EXPLAIN` command. For an overview of the new
  `EXPLAIN` syntax, check the [updated documentation](/self-managed/v2025.01/sql/explain-plan/).

* Include all indexes when running the [`SHOW INDEXES`](/self-managed/v2025.01/sql/show-indexes)
...
* Add a default index for all `SHOW` commands in the
  [`mz_introspection`](/self-managed/v2025.01/sql/show-clusters/#mz_catalog_server-system-cluster)
...
* Add the [`mz_introspection` system
  cluster](/self-managed/v2025.01/sql/show-clusters/#mz_catalog_server-system-cluster) to support
  efficiently serving common introspection queries.

* Add the [`mz_system` system
  cluster](/self-managed/v2025.01/sql/show-clusters/#mz_system-system-cluster) to support various
