...
* Support adding individual subsources in the [PostgreSQL source](/self-managed/v2025.01/sql/create-source/postgres/)
  using the new `ALTER SOURCE...ADD SUBSOURCE` syntax.

#### SQL

* Add the [`try_parse_monotonic_iso8601_timestamp`](/self-managed/v2025.01/sql/functions/pushdown/)
  function, which should be used in temporal filters involving `string` timestamps
  (e.g. extracted from `jsonb` columns) to benefit from [filter pushdown optimization](/self-managed/v2025.01/transform-data/patterns/temporal-filters/#temporal-filter-pushdown).
