...
* Allow reading from all non-errored subsources in the [PostgreSQL source](/self-managed/v2025.01/sql/create-source/postgres/),
...
* Automatically run introspection queries in the [`mz_introspection` cluster](/self-managed/v2025.01/sql/show-clusters/#mz_catalog_server-system-cluster),
  ...
  `auto_route_introspection_queries` [configuration parameter](/self-managed/v2025.01/sql/set/#other-configuration-parameters).
...
* Add `credits_per_hour` to the `mz_internal.mz_cluster_replica_sizes` system
  catalog table, and rate limit [free trial accounts](/self-managed/v2025.01/free-trial-faqs/) to 4
  credits per hour.
...
* Add the [`GRANT PRIVILEGE`](/self-managed/v2025.01/sql/grant-privilege) and [`REVOKE PRIVILEGE`](/self-managed/v2025.01/sql/revoke-privilege)
