...
* Add support for new SQL functions:
  ...
  | ----------------------------------------------- | ----------------------------------------------------------------------- |
  | [`ceiling`](/self-managed/v2025.01/sql/functions/#numbers-functions)       | Works as an alias of the `ceil` function.                               |

<br>

* Remove the `CREATE USER` command, as well as the `LOGIN` and `SUPERUSER`
  attributes from the [`CREATE ROLE`](/self-managed/v2025.01/sql/create-role/) command. This is part
