...
* Fix a bug that could cause updates in [sinks](/self-managed/v2025.01/sql/create-sink) to appear as
...
* Improve error message for unsupported types in the
  [PostgreSQL source](/self-managed/v2025.01/sql/create-source/postgres/), specifying the table and
  ...
  [Materialize types](/self-managed/v2025.01/sql/types/) is a work in progress {{% gh 15716 %}}.

* When using both signed and unsigned integers as inputs to a function, cast the
  inputs to a larger lossless type rather than [`double`](/self-managed/v2025.01/sql/types/float). For
  example, when determining equality between [`integer`](/self-managed/v2025.01/sql/types/integer)
  (32-bit signed integer) and [`uint4`](/self-managed/v2025.01/sql/types/uint) (32-bit unsigned
  integer), both values are now cast to [`bigint`](/self-managed/v2025.01/sql/types/integer)
  (64-bit signed integer). Previously both values would be cast to
  [`double`](/self-managed/v2025.01/sql/types/double) (64-bit floating point number).
