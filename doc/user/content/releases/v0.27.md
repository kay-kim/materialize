...
* Add [clusters](/self-managed/v2025.01/sql/create-cluster) and [cluster replicas](/self-managed/v2025.01/sql/create-cluster-replica/),
...
* Add [materialized views](/self-managed/v2025.01/sql/create-materialized-view), which are views that
  are persisted in durable storage and incrementally updated as new data
  arrives.

  A materialized view is created in a
  [cluster](/self-managed/v2025.01/concepts/clusters/) that
  ...
  create an [index](/self-managed/v2025.01/sql/create-index) on it. However, intermediate state
  necessary for efficient incremental updates of the materialized view may be
  maintained in memory.

* Add [connections](/self-managed/v2025.01/sql/create-connection/), which describe how to connect to
  and authenticate with external systems. Once created, a connection is reusable
  across multiple [`CREATE SOURCE`](/self-managed/v2025.01/sql/create-source) and
  [`CREATE SINK`](/self-managed/v2025.01/sql/create-sink) statements.

* Add [secrets](/self-managed/v2025.01/sql/create-secret), which securely store sensitive credentials
  (like passwords and SSL keys) for reference in connections.

* Durably record data ingested from [sources](/self-managed/v2025.01/sql/create-source).
...
* Allow provisioning the size of a source or sink.
  ...
  [`SIZE`](/self-managed/v2025.01/sql/create-source/#sizing-a-source) parameter.

* Add [load generator sources](/self-managed/v2025.01/sql/create-source/load-generator), which
  produce synthetic data for use in demos and performance tests.

* Add an [HTTP API](/self-managed/v2025.01/integrations/http-api) which supports executing SQL queries
  over HTTP.

* **Breaking change.** Require all [indexes](/self-managed/v2025.01/sql/create-index) to be associated
  with a cluster.

* **Breaking change.** Require the use of [connections](/self-managed/v2025.01/sql/create-connection/)
  with Kafka sources, PostgreSQL sources, and Kafka sinks.

* **Breaking change.** Rename `TAIL` to [`SUBSCRIBE`](/self-managed/v2025.01/sql/subscribe).
...
* **Breaking change.** Change the meaning of `CREATE MATERIALIZED VIEW`.
  ...
  [materialized view](/self-managed/v2025.01/sql/create-materialized-view), rather than providing a
...
* **Breaking change.** Remove the `reuse_topic` option from
  [Kafka sinks](/self-managed/v2025.01/sql/create-sink).
...
* **Breaking change.** Do not default to the [Debezium
  envelope](/self-managed/v2025.01/sql/create-sink/kafka/#debezium-envelope) in `CREATE SINK`. You
...
* **Breaking change.** Overhaul the system catalog.
  ...
  The relations in the [`mz_catalog`](/self-managed/v2025.01/sql/system-catalog/mz_catalog) schema
  ...
  [`mz_internal`](/self-managed/v2025.01/sql/system-catalog/mz_internal) schema. Those relations are
  not subject to the backwards compatibility policy.

* **Breaking change.** Rename `mz_logical_timestamp()` to [`mz_now`](/self-managed/v2025.01/sql/functions/now_and_mz_now/).
