...
* Support upserts in the output of `SUBSCRIBE` via the new [`ENVELOPE UPSERT` clause](/self-managed/v2025.01/sql/subscribe/#envelope-upsert).
...
* Add [`mz_internal.mz_compute_dependencies`](/self-managed/v2025.01/sql/system-catalog/mz_internal/#mz_compute_dependencies)
...
* Improve the output of [`EXPLAIN { OPTIMIZED | PHYSICAL } PLAN FOR MATERIALIZED VIEW`](/self-managed/v2025.01/sql/explain-plan/)
...
* Add support for [`TABLE`](/self-managed/v2025.01/sql/table) expressions, which retrieve all rows
