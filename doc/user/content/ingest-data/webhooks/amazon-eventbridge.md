...
in Materialize to ingest data from Amazon EventBridge. By default, the table
will be created in the active cluster; to use a different cluster, run
[`SET CLUSTER`](/sql/set/) first. (The legacy `CREATE SOURCE` syntax also
accepts an `IN CLUSTER` clause.)

{{< tabs >}}
{{< tab "New Syntax" >}}

```mzsql
CREATE TABLE eventbridge_source
FROM WEBHOOK
  BODY FORMAT JSON
...
If your throughput exceeds the [maximum request rate](/sql/create-table/webhook/#request-limits),
we recommend [batching multiple events](/sql/create-table/webhook/#handling-batch-events)
...
With the table set up in Materialize and the API destination configured in
